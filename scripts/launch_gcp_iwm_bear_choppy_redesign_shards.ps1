param(
    [string]$WaveId = "ticker365_iwm_bear_choppy_redesign_20260505T1505Z",
    [string]$InstanceSuffix = "20260505a",
    [int]$CandidateCountPerWorker = 6,
    [int]$TotalCandidates = 72,
    [int]$TopN = 72,
    [int]$MaxLaunches = 12,
...
$InputVariantsUri = "$GcsPrefix/inputs/iwm_bear_choppy_redesign_variants.jsonl"
$InputQueueUri = "$GcsPrefix/inputs/iwm_bear_choppy_redesign_option_queue.json"
$LaunchRowsUri = "$GcsPrefix/ops/iwm_bear_choppy_redesign_launch_rows.json"
...
$LaunchRowsPath = Join-Path $ReportDir "iwm_bear_choppy_redesign_launch_rows.json"
$SourceArchivePath = Join-Path $env:TEMP "$WaveId-codexalpaca_repo_source.tar.gz"
$VariantPath = Join-Path $InputsDir "iwm_bear_choppy_redesign_variants.jsonl"
$QueuePath = Join-Path $InputsDir "iwm_bear_choppy_redesign_option_queue.json"
$ManifestPath = Join-Path $InputsDir "iwm_bear_choppy_redesign_manifest.json"
...
function Invoke-GcloudCreateInstance {
    ...
    if ($message -match "CPUS_ALL_REGIONS" -or $message -match "Quota .* exceeded" -or $message -match "ZONE_RESOURCE_POOL_EXHAUSTED") {
...
& $Python scripts\build_iwm_bear_choppy_redesign_research_inputs.py `
    --symbol IWM `
    --wave-id $WaveId `
...
Invoke-Gcloud @("storage", "cp", $ManifestPath, "$GcsPrefix/inputs/iwm_bear_choppy_redesign_manifest.json", "--project", $Project)
...
for ($candidateStart = 1; $candidateStart -le $TotalCandidates; $candidateStart += $CandidateCountPerWorker) {
    ...
    $workerId = "iwm_bc_red_c${startSlug}_${endSlug}"
    $instanceName = "iwm-bc-red-c$startSlug-$endSlug-$InstanceSuffix"
...
foreach ($row in $launchRows) {
    ...
    $metadata = @{
        ...
        profile_name = "iwm-bear-choppy-redesign-strict-e0x60"
    ...
    $createStatus = Invoke-GcloudCreateInstance @(
        ...
        "--labels", "app=codexalpaca,env=research,role=iwm-redesign,wave=iwm-bc-red,symbol=iwm",
