...
$LogPath = Join-Path $LogDir "ticker365_paper_readiness_watchdog.log"
...
Add-WatchdogLogLine "===== ticker365_paper_readiness_watchdog run $stamp ====="

& $Python "scripts\monitor_ticker365_paper_readiness.py" --gcloud-bin $Gcloud --once 2>&1 |
    ForEach-Object {
        $line = $_.ToString()
        Write-Output $line
