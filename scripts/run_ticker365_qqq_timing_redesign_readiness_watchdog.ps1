...
$LogPath = Join-Path $LogDir "ticker365_qqq_timing_redesign_readiness_watchdog.log"
$LockPath = Join-Path $LogDir "ticker365_qqq_timing_redesign_readiness_watchdog.lock"
...
$OutputDir = "reports/gcp_research/ticker365_qqq_timing_redesign_20260504/paper_readiness"
...
try {
    ...
    "===== ticker365_qqq_timing_redesign_readiness_watchdog skipped_locked $stamp =====" | Out-File -FilePath $LogPath -Append -Encoding utf8
    exit 0
}
...
try {
    $stamp = (Get-Date).ToUniversalTime().ToString("yyyy-MM-ddTHH:mm:ssZ")
    Add-WatchdogLogLine "===== ticker365_qqq_timing_redesign_readiness_watchdog run $stamp ====="
