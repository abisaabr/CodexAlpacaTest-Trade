...
$LogPath = Join-Path $LogDir "ticker365_entry_asof_rescue_readiness_watchdog.log"
$LockPath = Join-Path $LogDir "ticker365_entry_asof_rescue_readiness_watchdog.lock"
...
$OutputDir = "reports/gcp_research/ticker365_entry_asof_rescue_paper_readiness_20260503"
...
try {
    ...
    "===== ticker365_entry_asof_rescue_readiness_watchdog skipped_locked $stamp =====" | Out-File -FilePath $LogPath -Append -Encoding utf8
    exit 0
}
...
try {
    $stamp = (Get-Date).ToUniversalTime().ToString("yyyy-MM-ddTHH:mm:ssZ")
    Add-WatchdogLogLine "===== ticker365_entry_asof_rescue_readiness_watchdog run $stamp ====="
