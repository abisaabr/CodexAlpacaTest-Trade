...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Build a compact QQQ bull option-native exit research queue."
    )
    parser.add_argument("--symbol", default="QQQ")
    parser.add_argument("--output-dir", required=True)
    parser.add_argument("--wave-id", default="ticker365_qqq_bull_premium_exit_20260505")
...
def _debit_exit_profiles() -> list[dict[str, Any]]:
    return [
        {
...
def build_bull_premium_exit_rows(*, symbol: str, wave_id: str) -> list[dict[str, Any]]:
    rows: list[dict[str, Any]] = []
    for row in build_rows(symbol=symbol, wave_id=wave_id):
        regime, direction, family = _source_parts(row)
        if regime != "bull" or direction != "call" or family != "single_leg_repair":
            continue
        base_parameters = dict(row["parameters"])
        for profile in _debit_exit_profiles():
...
def main() -> None:
    ...
    rows = build_bull_premium_exit_rows(symbol=symbol, wave_id=args.wave_id)
    ...
    manifest = {
        ...
        "status": "ready_for_qqq_bull_premium_exit_backtest",
        "target_regimes": ["bull"],
    ...
    variants_path = output_dir / "qqq_bull_premium_exit_variants.jsonl"
    queue_path = output_dir / "qqq_bull_premium_exit_option_queue.json"
    manifest_path = output_dir / "qqq_bull_premium_exit_manifest.json"
