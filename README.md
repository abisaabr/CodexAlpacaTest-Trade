...
Portable, public, paper-trading-first Alpaca research and execution lab for:
...
- Live trading is refused by configuration and by the broker gateway.
- All broker calls route through `alpaca_lab/brokers/alpaca.py`.
- Paper execution defaults to dry-run previews and requires an explicit submit flag to send paper orders.
...
Copy `.env.example` to `.env`, then fill in paper-only credentials locally.
